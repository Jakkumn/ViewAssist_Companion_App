...
class VAWyomingService:
    """Hold info for Wyoming service."""

    def __init__(
        self, host: str, port: int, info: Info, capabilities: dict[str:Any]
    ) -> None:
    ...
    @classmethod
    async def create(cls, host: str, port: int) -> VAWyomingService | None:
        """Create a Wyoming service."""
        info, capabilities = await load_wyoming_info(host, port)
        if info is None or capabilities is None:
            return None

        return cls(host, port, info, capabilities)
...
async def load_wyoming_info(
        ...
        try:
            async with AsyncTcpClient(host, port) as client, asyncio.timeout(timeout):
                ...
                if wyoming_info is not None:
                    await client.write_event(Capabilities().event())
                    while True:
                        event = await client.read_event()
                        if event is None:
                            raise WyomingError(  # noqa: TRY301
                        ...
                        if Capabilities.is_type(event.type):
    ...
    return wyoming_info, capabilities
